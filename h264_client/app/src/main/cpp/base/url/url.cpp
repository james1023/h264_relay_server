...
inline bool is_scheme(const char *s, const char *e)
{
	...
	if ((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z')) {
		...
		DEBUG_TRACE(JAM_DEBUG_LEVEL_ERROR, "#%d url::is_scheme, s=%s, e=%s, c=%c, s=%#x, e=%#x. \n", __LINE__, s, e, c, s, e);
		return false;
	}
	...
	if (s == e) {
		DEBUG_TRACE(JAM_DEBUG_LEVEL_ERROR, "#%d url::is_scheme, s=%s, e=%s, s=%#x, e=%#x. \n", __LINE__, s, e, s, e);
		return false;
	}
	...
	while (++s != e) {
		if (!is_alnum(c = *s) && c != '+' && c != '-' && c != '.') {
			DEBUG_TRACE(JAM_DEBUG_LEVEL_ERROR, "#%d url::is_scheme, s=%s, e=%s, c=%c. \n", __LINE__, s, e, c);
			return false;
		}
	}
...
Url &Url::port(const std::string& p) {
	...
	if ((m_scheme == "http" && o == "80") || (m_scheme == "https" && o == "443")) {
		o = "";
		port_ = 80;
		if (m_scheme == "https" && o == "443") port_ = 443;
	}
	else {
		try {
            //port_ = boost::lexical_cast<unsigned>(o.c_str());
            port_ = (unsigned)stoi(o);
        }
        catch (...) {
			if (m_scheme == "rtsp")
				port_ = 554;
...
void Url::parse_url() {
    ...
    std::string _scheme, _user, _host, _port, _path, _query, _fragment, _uri;
    Query query_v;
        ...
        while (p!=query_e) {
            ...
            if (p!=query_e) {
                if (*p=='=') {
					...
					// 20160914 james: don't check query value.
#if 1
#else
                    if (!is_chars(b, p, 0x3F))
	...
	try { 
		//port_ = boost::lexical_cast<unsigned>(_port);
        port_ = (unsigned)std::stoi(_port);
	}
	catch (...) { 
